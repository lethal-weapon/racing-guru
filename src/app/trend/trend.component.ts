...
import {EarningStarter, Meeting, PlayerSummary} from '../model/meeting.model';
...
export class TrendComponent implements OnInit {
  ...
  //     .find(p => p.player === this.activePerson)
  //     ?.starters
  ...
  //
  //   const persons = meetings[0].players.filter(p => p.player == person);
  //   if (persons.length !== 1) {
  ...
  //     .filter(m => {
  //       const engaged = m.players.map(p => p.player).includes(person);
  //       if (!engaged) return false;
  //       return m.players.find(p => p.player === person)?.wins || 0 > 0;
  //     });
  //
  ...
  //     const summary = this.meetings[i].players.find(p => p.player === person);
  //     if (!summary) continue;
  //
  //     if (summary.wins === 0) {
  ...
  //     .filter((_, index) => index > 0)
  //     .filter(m => m.players.map(p => p.player).includes(person))
  //     .sort((m1, m2) => m1.meeting.localeCompare(m2.meeting));
  //
  //   let gap: number = 0;
  //   for (let i = 0; i < sortedMeetings.length; i++) {
  //     const summary = sortedMeetings[i].players.find(p => p.player === person);
  //     if (!summary) continue;
  //
  //     if (summary.wins === 0) {
  ...
  //
  //   this.meetings
  //     .map(m => m.players.filter(p => p.player === this.activePerson && p.earnings > 0))
  ...
  //
  //   const sortedMeetings = this.meetings
  //     .filter(m => m.players.map(p => p.player).includes(this.activePerson))
  //     .sort((m1, m2) => m1.meeting.localeCompare(m2.meeting));
  //
  //   let legs: MilestoneWinnerLeg[] = [];
  //   for (let i = 0; i < sortedMeetings.length; i++) {
  //     sortedMeetings[i].players
  //       .filter(ps => ps.player === this.activePerson)
